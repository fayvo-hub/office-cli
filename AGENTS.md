...
- Git:两个远程。发布/提交只推 `github`(fayvo-hub/office-cli);`origin` 是阿里云 codeup 旧远程,勿推。直连 GitHub 被重置,推送用 `git -c http.proxy=http://127.0.0.1:7897 push github master`(`-c` 在子命令前)。
